...
8. **Relationship Handler Behavior (inline summary):**
   Include a brief inline explanation of how relationship handlers work (balances 200-line constraint with locked decision):
   - OneToMany (BidirectionalOneToManyHandler): Translates collection items, maintains inverse property pointing back to parent
   - ManyToOne: Creates new relation pointing to translated target or same shared entity
   - ManyToMany: Clones association collection for the translation
   - "For complete handler chain details and priority order, see llms.md 'Handler Chain Decision Tree' section"
...
- Include the inline relationship handler summary (3-4 lines) before referencing llms.md for full details
  </action>
  <verify>
1. File exists: `test -f .claude/skills/entity-translation-setup/SKILL.md && echo "EXISTS" || echo "MISSING"`
2. Line count under 200: `wc -l < .claude/skills/entity-translation-setup/SKILL.md`
3. Frontmatter valid: `head -10 .claude/skills/entity-translation-setup/SKILL.md`
4. Trigger phrases present: `grep -c "make.*translatable\|add translations" .claude/skills/entity-translation-setup/SKILL.md`
5. Relationship handler inline summary: `grep -c "OneToMany\|ManyToOne" .claude/skills/entity-translation-setup/SKILL.md`
6. llms.md reference: `grep -c "llms.md" .claude/skills/entity-translation-setup/SKILL.md`
  </verify>
  <done>
...
- Inline relationship handler behavior summary (OneToMany, ManyToOne, ManyToMany)
- Diff-style code template
- User confirmation step
- Migration command reminder
- Reference to llms.md for complete handler chain details
  </done>
</task>
...
3. **Workflow completeness check:**
   ...
   - [ ] Relationship handler inline summary (OneToMany, ManyToOne, ManyToMany behavior)
...
5. **Content accuracy check:**
  ...
  <verify>
Validation performed during action execution - each checklist item above is verified by reading the file and confirming presence/correctness. Final validation commands:

```bash
# Frontmatter structure
head -10 .claude/skills/entity-translation-setup/SKILL.md

# Line count
wc -l < .claude/skills/entity-translation-setup/SKILL.md

# Trigger phrases
grep -c "make.*translatable\|add translations" .claude/skills/entity-translation-setup/SKILL.md

# Both attributes mentioned
grep -c "SharedAmongstTranslations" .claude/skills/entity-translation-setup/SKILL.md
grep -c "EmptyOnTranslate" .claude/skills/entity-translation-setup/SKILL.md

# Relationship handler inline summary
grep -c "OneToMany\|ManyToOne\|ManyToMany" .claude/skills/entity-translation-setup/SKILL.md

# llms.md reference
grep -c "llms.md" .claude/skills/entity-translation-setup/SKILL.md
```
  </verify>
  <done>
Skill validated:
...
- Relationship handler behavior explained inline
- Correct import paths
- Reference to llms.md present for complete details
  </done>
</task>
...
1. **Requirement SKILL-01 coverage:**
   ...
   - [x] Inline relationship handler behavior summary
   - [x] References llms.md for complete handler details
...
2. **User decisions honored (from CONTEXT.md):**
   ...
   - [x] Detailed inline explanation of relationship handler chain behavior
...
- SharedAmongstTranslations examples: SKU, price, date, category
- EmptyOnTranslate examples: slug, SEO URL
- Inline relationship handler summary (OneToMany, ManyToOne, ManyToMany)
- Diff-style code template with + markers present
- User confirmation step included
- Migration command reminder included
- llms.md referenced for complete handler chain details
</success_criteria>
