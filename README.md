...
- **Diagnostics** — `tmi:translation:doctor` fails on broken linkage (orphan, duplicate, `null-tuuid`) and lists untranslated and incomplete records for information (`--strict` counts those too), for every entity or, with `--entity`, just one; `tmi:translation:sync-shared` names every drifted `#[SharedAmongstTranslations]` property, how many Tuuid groups and rows it touched, and whether it was writable, lists every changed value with `-v`, and asks before a whole-table write on an interactive terminal (`-n` in scripts); `strict_discovery` fails the container compile, instead of only logging, when compile-time attribute discovery finds zero translatable entities.
...
* **Unique constraints need the locale column.** A single-column `unique: true` on a translatable field fails the moment Doctrine loads the mapping (first request, console command, schema tool alike) — the same value legitimately repeats once per locale. Use a composite constraint (`field + locale`) instead — see [Quick Fix for unique fields](#quick-fix-for-unique-fields).
...
* **Sharing inside a nested embeddable is not propagated on flush.** An embeddable within an embeddable is translated like any value, but a `#[SharedAmongstTranslations]` on the inner one is honoured at `translate()` time only — declare sharing on the entity's own embedded property instead.
